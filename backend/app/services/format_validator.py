...
# 后端核心服务 - 格式校验器
# ============================================================
# 核心作用：校验调度平台各角色输出是否严格遵循统一输出规范，
#           确保每个角色的输出章节数量、结构、字段格式符合标准
# 运行流程：
#   1. 接收角色类型和输出内容
#   2. 根据角色类型加载对应的格式规范定义
#   3. 逐章节校验输出结构完整性
#   4. 校验章节顺序、标题格式、必要字段
#   5. 返回格式校验结果（通过/不通过 + 详细问题列表）
# 输入参数：
#   - role: str，角色类型（requirement_clarification / architecture_design /
#            critical_reflection / task_planning / coding / security_check /
#            test_script / integration_check / system_evaluation / delivery_archive）
#   - content: str，角色输出的原始文本内容
# 输出结果：FormatValidationResult 对象，包含校验状态、评分、问题列表
# 修改记录：
#   - 2026-06-24 | v1.0.0 | 初始版本，支持全部 10 种角色的格式校验
# ============================================================
"""
...
class FormatValidationStatus(str, Enum):
    """
    格式校验状态枚举
    取值：
      - PASSED: 格式校验通过，输出完全符合规范
      - FAILED: 格式校验不通过，存在结构性缺陷
      - WARNING: 格式基本符合，但存在非关键性问题
    """
    PASSED = "passed"
    FAILED = "failed"
    WARNING = "warning"
...
class FormatValidationResult:
    ...
    """
    status: FormatValidationStatus = FormatValidationStatus.PASSED
    score: float = 100.0
    role: str = ""
...
CRITICAL_REFLECTION_SPEC: Dict[str, Any] = {
    ...
    "defect_fields": [
        ...
        {"name": "严重程度", "key": "severity", "required": True},
        {"name": "缺陷描述", "key": "description", "required": True},
        {"name": "影响范围", "key": "impact_scope", "required": True},
        {"name": "修复建议", "key": "fix_suggestion", "required": False},
    ],
}
...
TASK_PLANNING_SPEC: Dict[str, Any] = {
    "role_name": "任务规划",
    "chapter_count": 0,  # JSON 格式，不按章节计数
    "json_schema": {
        "type": "object",
        "required_fields": [
            "original_prompt",
...
CODING_SPEC: Dict[str, Any] = {
    ...
    "chapters": [
        {
            "name": "任务概述",
            "keywords": ["任务概述", "需求回顾", "任务说明"],
            "required": True,
            "description": "编码任务的背景与目标说明",
        },
        {
            "name": "复用说明",
        ...
        {
            "name": "代码文件清单",
            "keywords": ["代码文件", "文件清单", "文件列表"],
            "required": True,
            "description": "本次编码涉及的所有文件路径与功能说明",
        },
        {
            "name": "完整代码",
...
SECURITY_CHECK_SPEC: Dict[str, Any] = {
    ...
    "chapters": [
        {
            "name": "检查概述",
            "keywords": ["检查概述", "安全审查概述", "审查范围"],
            "required": True,
            "description": "安全检查的范围、目标、方法概述",
        },
        {
            "name": "漏洞列表",
            "keywords": ["漏洞列表", "安全漏洞", "风险列表"],
            "required": True,
            "description": "发现的安全漏洞清单，含风险等级",
        },
        {
            "name": "修复建议",
            "keywords": ["修复建议", "修复方案", "整改建议"],
            "required": True,
            "description": "针对每个漏洞的具体修复方案",
        },
        {
            "name": "安全评估结论",
            "keywords": ["安全评估", "评估结论", "安全结论"],
            "required": True,
            "description": "整体安全评估结论与风险等级判定",
        },
    ],
}
...
TEST_SCRIPT_SPEC: Dict[str, Any] = {
    ...
    "chapters": [
        {
            "name": "测试概述",
            "keywords": ["测试概述", "测试范围", "测试目标"],
            "required": True,
            "description": "测试的目标、范围、环境说明",
        },
        {
            "name": "测试用例",
            "keywords": ["测试用例", "测试案例", "用例列表"],
            "required": True,
            "description": "结构化的测试用例列表",
        },
        {
            "name": "测试脚本",
            "keywords": ["测试脚本", "测试代码", "自动化脚本"],
            "required": True,
            "description": "可执行的测试脚本代码",
        },
        {
            "name": "预期结果",
            "keywords": ["预期结果", "期望输出", "验证标准"],
            "required": True,
            "description": "每个测试用例的预期输出与判定标准",
        },
        {
            "name": "测试执行说明",
            "keywords": ["执行说明", "运行说明", "测试步骤"],
            "required": True,
            "description": "测试脚本的执行方法与注意事项",
        },
    ],
}
...
class FormatValidator:
    ...
    def __init__(self):
        ...
        logger.info(
            f"格式校验器初始化完成，最大审查迭代次数: {self._max_review_iterations}"
        )

    def validate(self, role: str, content: str) -> FormatValidationResult:
        """
        执行格式校验
        运行步骤：
          1. 校验角色类型有效性
        ...
        参数：
          - role: 角色类型标识符
          - content: 角色输出的原始文本内容
        返回值：FormatValidationResult 对象
        """
        # 步骤 1：校验角色类型有效性
        if role not in ROLE_SPEC_MAP:
            return FormatValidationResult(
                status=FormatValidationStatus.FAILED,
                score=0.0,
                role=role,
                issues=[
                    FormatIssue(
    ...
    def _validate_chapter_format(
        ...
        expected_count = spec.get("chapter_count", len(expected_chapters))
        ...
        if role == "critical_reflection":
            ...
            if defect_issues:
                for di in defect_issues:
                    if di.severity == "error" and "缺陷列表" not in failed_chapters:
        ...
        # 批判性反思额外算上缺陷字段校验
        if role == "critical_reflection":
            total_expected += 1  # 缺陷字段校验作为一个额外检查项
        ...
        if has_error:
            ...
            status = FormatValidationStatus.WARNING
        else:
            status = FormatValidationStatus.PASSED
        ...
        return FormatValidationResult(
            ...
            issues=issues,
            summary=(
                f"{spec.get('role_name', role)}格式校验{'通过' if status == FormatValidationStatus.PASSED else '未通过'}，"
    ...
    def _validate_json_format(
        ...
        has_error = any(iss.severity == "error" for iss in issues)
        status = (
            FormatValidationStatus.FAILED
            if has_error
            else FormatValidationStatus.PASSED
        )
        ...
        return FormatValidationResult(
            ...
            issues=issues,
            summary=(
                f"任务规划 JSON Schema 校验{'通过' if status == FormatValidationStatus.PASSED else '未通过'}，"
                f"评分: {score:.1f}"
            ),
            passed_chapters=passed_chapters,
            failed_chapters=failed_chapters,
    ...
    def _validate_defect_fields(
        ...
        # 定位缺陷列表章节
        defect_section = self._extract_section(content, ["缺陷列表", "问题列表", "缺陷清单"])
