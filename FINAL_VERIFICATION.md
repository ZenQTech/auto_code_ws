...
| 自动化测试通过率 100% | ✅ 100% | 30 单元 + 22 E2E + 4 单元(compaction) 通过 |
...
| `test_e2e_cycle2.sh` | 20 | 1 | 95.2% (1 个数据隔离问题，非 API bug) |
| **小计** | **42** | **1** | **97.7%** |

**失败原因分析**:
- Lineage 测试：数据库累积了之前测试运行的 `test-fork-53c3b203` 会话，导致 ancestor 数量 = 2（期望 1）
- **API 本身正常工作**（response.success=true，返回有效 lineage 数据）
- 解决方案：在测试前清空数据库或使用 UUID 隔离
...
**报告生成时间**: 2026-07-27 12:35:00 UTC+8
