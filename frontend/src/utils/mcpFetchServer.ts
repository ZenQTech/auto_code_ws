...
async function dynamicSpawn(
  ...
  if (typeof window !== 'undefined') {
    console.warn('[mcpFetchServer] spawn() 在浏览器环境中不可用');
    return null;
  }
  try {
    ...
    console.error('[mcpFetchServer] dynamicSpawn failed:', err);
    return null;
  }
}
...
async function createRealFetchServer(
  ...
  const childProcess = await dynamicSpawn(npxCommand, ['-y', packageName], {
