/**
 * # ============================================================
 * LoopStateMachineView - Loop 状态机可视化 (v1.0.0)
 * Cycle 58 G58-03
 * # ============================================================
  ...
  return (
    ...
    >
      ...
      <div className="mb-3 p-3 bg-surface-50 rounded-lg">
        <div className="text-xs text-surface-500 mb-1">当前阶段</div>
        <div className="text-base font-semibold text-surface-800 flex items-center gap-2">
          <div
      ...
      {/* 状态机迁移图 */}
      <div className="mb-3 border border-surface-200 rounded-lg p-2 bg-slate-50">
          ...
          {STAGES.map((stage) => {
            ...
            return (
              <g key={stage.key} transform={`translate(${stage.x}, ${stage.y})`}>
                <circle
                  r={isCurrent ? 22 : 18}
                  fill={isCurrent ? stage.color : '#fff'}
                  stroke={stage.color}
      ...
      {/* 迁移历史 */}
      <div>
        <div className="text-xs font-medium text-surface-700 mb-2">最近迁移</div>
        {recentHistory.length === 0 ? (
          <div className="text-xs text-surface-400">无迁移记录</div>
        ) : (
          <div className="space-y-1 max-h-40 overflow-y-auto" data-testid="loop-history">
            {recentHistory.map((t, i) => (
              ...
              >
                <span className="text-surface-500 font-mono w-6">{history.length - i}</span>
                <span className="text-surface-600">{t.from_state}</span>
                <span className="text-surface-400">→</span>
                <span className="text-surface-800 font-medium">{t.to_state}</span>
                <span className="ml-auto text-surface-400 font-mono">
