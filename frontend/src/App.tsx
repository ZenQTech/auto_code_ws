...
export default function App() {
  ...
  return (
    ...
    <div className="min-h-screen bg-[var(--bg-app)] flex">
      ...
      {/* v6.118.0 Cycle 44 G44-04 新增：MCP 多模态智能体面板弹窗
       * 触发：BrandHeader 菜单"MCP 多模态智能体"项
       * 关闭：McpMultimodalPanel 内部 onClose 回调
       * 功能：图像/音频/文件多模态端到端处理
      ...
      {/* v6.122.0 Cycle 48 G48-主应用集成 新增：MCP × 多模态 RAG 面板弹窗
       * 触发：BrandHeader 菜单"🎨 MCP × 多模态 RAG"项
       * 关闭：McpMultimodalRagPanel 内部 onClose 回调
       * 功能：5 Tab（多模态 Embedding / 图文混合索引 / 跨模态缓存 / 性能基准 / 系统设置）
      ...
      {/* v6.140.0 Cycle 56 G56-主应用集成 新增：MCP × Serverless/FaaS 集成面板弹窗
       * 触发：BrandHeader 菜单"☁️ MCP × Serverless"项
       * 关闭：McpServerlessPanel 内部 onClose 回调
       * 功能：5 Tab（Knative / KEDA / OpenFaaS / CloudEvents / 集成文档）
